...
function wdja_cms_module_orderdisp()
{
  ...
  $tutype = ap_get_userinfo('utype', $nusername);//会员级别
  if ($tnutype == 1) $tprice = 180;
  else $tprice = 380;
  if ($tutype == 1 && $tnutype == 2) $tprice = 200 + 80;//如果是升级会员,只需补差价+80元即可.
...
function wdja_cms_module_state()
{
  global $conn;
  global $nusername;
  global $ndatabase, $nidfield, $nfpre;
  ...
  if ($trs)
  {
    ...
    $tmpstr = ii_itake('module.state', 'tpl');
    foreach ($trs as $key => $val)
    {
      $tkey = ii_get_lrstr($key, '_', 'rightr');
      $GLOBALS['RS_' . $tkey] = $val;
