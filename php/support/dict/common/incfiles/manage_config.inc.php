...
function pp_manage_subnum($id) {
  global $slng, $mgroup;
  global $conn;
  global $ndatabase, $nidfield, $nfpre;
  $tsqlstr = "select count(*) as count from $ndatabase where " . ii_cfname('fsid') . "='$id' and " . ii_cfname('group') . "='$mgroup' and " . ii_cfname('lng') . "='$slng'";
  $trs = ii_conn_query($tsqlstr, $conn);
  $trs = ii_conn_fetch_array($trs);
  if ($trs) return $trs['count'];
}
...
function wdja_cms_admin_manage_action()
{
  ...
  switch($taction)
  {
    ...
    case 'reset':
      ...
      break;
    case 'order':
      wdja_cms_admin_orderdisp('support.dict', '', " and " . ii_cfname('group') . "='" . $mgroup . "' and " . ii_cfname('lng') . "='" . $slng . "'");
      break;
    case 'control':
...
function wdja_cms_admin_manage_edit()
{
  ...
  if ($trs)
  {
    ...
    $tmpstr = str_replace('{$nav_dict}', mm_nav_dict($mgroup, '?group=' . $mgroup . '&id=', $tid), $tmpstr);
...
function wdja_cms_admin_manage_list()
{
  ...
  if (is_array($trsary))
  {
    foreach($trsary as $trs)
    {
      ...
      $tmptstr = str_replace('{$sub}', ii_get_num(pp_manage_subnum($trs[$nidfield]),0), $tmptstr);
      $tmptstr = str_replace('{$group}', ii_itake('sel_group.'.ii_htmlencode($trs[ii_cfname('group')]),'sel'), $tmptstr);
      $tmptstr = str_replace('{$alt}', ii_htmlencode($trs[ii_cfname('alt')]), $tmptstr);
      $tmptstr = str_replace('{$order}', ii_get_num($trs[ii_cfname('order')]), $tmptstr);
