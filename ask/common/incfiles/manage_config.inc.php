...
function wdja_cms_admin_manage_adddisp()
{
  ...
  $tcontent_atts_list = ii_left(ii_cstr($_POST['content_atts_list']), 100000);
...
function wdja_cms_admin_manage_view()
{
  ...
  if ($trs)
  {
    ...
    $tmpstr = str_replace('{$upsimg}', $nupsimg, $tmpstr);
    $tmpstr = str_replace('{$upsimgs}', $nupsimgs, $tmpstr);
    $tmpstr = str_replace('{$list_data}', wdja_cms_admin_manage_list_answer($trs[$nidfield]), $tmpstr);
    $tmpstr = ii_creplace($tmpstr);
    return $tmpstr;
  }
  else mm_client_alert(ii_itake('global.lng_public.sudd', 'lng'), -1);
...
function wdja_cms_admin_manage_list_answer($fid)
{
  global $conn, $slng, $variable;
  global $nclstype, $npagesize, $nlisttopx;
  $ngenre = 'ask/answer';
  ...
  $tmprstr = '';
  $tsqlstr = "select * from $ndatabase_data where " . ii_cfnames($nfpre_data,'lng') . "='$slng' and  ". ii_cfnames($nfpre_data,'fid') . "='$fid'";
  ...
  if (is_array($trsary))
  {
    ...
    {
      $tmptstr = str_replace('{$fid}', ii_get_num($trs[ii_cfnames($nfpre_data,'fid')]), $tmpastr);
      $tmptstr = str_replace('{$author}', ii_htmlencode($trs[ii_cfnames($nfpre_data,'author')]), $tmptstr);
      $tmptstr = str_replace('{$content}', ii_left(ii_cstr($trs[ii_cfnames($nfpre_data,'content')]),100), $tmptstr);
      $tmptstr = str_replace('{$good}', ii_itake('global.sel_yesno.'.ii_get_num($trs[ii_cfnames($nfpre_data,'good')]), 'sel'), $tmptstr);
