...
function mm_nav_topic($fsid)
{
  ...
  foreach($tfidary as $fid)
  {
    if($fid !=0){
        ...
        if($ttype == 0) $tstr = str_replace('{$value}', '?type=list&classid=' . $fid .'', $tstr);
        $tstr = str_replace('{$value}', '?type=detail&id=' . $fid .'', $tstr);
...
function wdja_cms_module_list()
{
  ...
  if (is_array($trsary))
  {
    foreach($trsary as $trs)
    {
      ...
      $turl = ii_iurl('list', $trs[$nidfield], $nurltype, 'folder=' . $ncreatefolder . ';filetype=' . $ncreatefiletype . ';time='.$trs[ii_cfname('time')]);
