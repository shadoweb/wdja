...
function wdja_cms_module_adddisp()
{
  ...
  if (!is_array($Err))
  {
    ...
    if ($trs) {
        $gmail = ii_itake('global.' . ADMIN_FOLDER . '/blog:extend.message_mail','lng');
        $gtitle = ii_itake('global.' . ADMIN_FOLDER . '/blog:extend.message_title','lng');
        $gbody = ii_itake('global.' . ADMIN_FOLDER . '/blog:extend.message_body','lng');
