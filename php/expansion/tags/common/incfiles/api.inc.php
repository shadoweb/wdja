...
function api_tags_all($num='20') {
  ...
  $nurltype = $variable[ii_cvgenre($tgenre) . '.nurltype'];
  $ncreatefolder = $variable[ii_cvgenre($tgenre) . '.ncreatefolder'];
  $ncreatefiletype = $variable[ii_cvgenre($tgenre) . '.ncreatefiletype'];
  $tmptstr = '';
  $tmpstr = ii_itake('global.expansion/tags:api.api_tags_all', 'tpl');
  ...
  while ($trow = ii_conn_fetch_array($trs))
  {
    ...
    else $tmptstr = str_replace('{$turl}', '/'.$tgenre.'/'.ii_iurl('detail', $tid, $nurltype, 'folder=' . $ncreatefolder . ';filetype=' . $ncreatefiletype), $tmpstr);
...
function api_tags_genre_list($genre,$num='10') {
  ...
  $tmpstr = ii_itake('global.expansion/tags:api.api_tags_genre_list', 'tpl');
  $tmpastr = ii_ctemplate($tmpstr, '{@}');
  $tmptstr = '';
  $tmprstr = '';
  $tsqlstr = "select * from $ndatabase_genre where ".$nidfield_genre  . " in (". api_tags_get_gids($genre,$tid) .") and ".ii_cfnames($nfpre_genre,'hidden')."=0 order by ".ii_cfnames($nfpre_genre,'time')." desc limit 0,".$tnum;
  $trs = ii_conn_query($tsqlstr, $conn);
  while ($trow = ii_conn_fetch_array($trs))
  {
    ...
    $tmptstr = str_replace('{$createfolder}', $ncreatefolder, $tmptstr);
...
{
  //标签数据转成标签和标签链接组成的数组
  global $conn, $ngenre, $nlng, $variable;
  $tgenre = 'expansion/tags';
  $tdatabase = mm_cndatabase(ii_cvgenre($tgenre));
  $tidfield = mm_cnidfield(ii_cvgenre($tgenre));
  $tfpre = mm_cnfpre(ii_cvgenre($tgenre));
  ...
  else
  {
    ...
    for($i=0;$i<$tnum;$i++)
    {
      ...
      if (!ii_isnull($tgourl)) $turl = '<a href="'.$tgourl.'">'.$ttopic.'</a>';
      else $turl = '<a href="/'.$tgenre.'/'.ii_iurl('detail', $tid, $nurltype, 'folder=' . $ncreatefolder . ';filetype=' . $ncreatefiletype).'">'.$ttopic.'</a>';
      $nrs[$ttopic] = &$turl;
      unset($trs[$i]);
      unset($tid);
      unset($tgourl);
...
function api_tags_list($id='',$genre='') {
  ...
  $trs = ii_conn_fetch_array($trs);
  if ($trs) {
    $tstrplus = ii_itake('global.expansion/tags:api.tags','lng').':';
    $tids = $trs[ii_cfnames($tfpre,"tid")];
    ...
    foreach ($tids_array as $key => $val)
    {
      ...
      if (!ii_isnull($tgourl)) $tmptstr = str_replace('{$turl}', $tgourl, $tmpstr);
      else $tmptstr = str_replace('{$turl}', '/'.$tgenre.'/'.ii_iurl('detail', $val, $nurltype, 'folder=' . $ncreatefolder . ';filetype=' . $ncreatefiletype), $tmpstr);
      $tmptstr2 .= str_replace('{$tag}', $tag, $tmptstr).',';
    }
