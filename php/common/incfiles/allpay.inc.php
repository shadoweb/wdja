...
function wxpay($out_trade_no,$subject,$total_amount,$body) {
  ...
  $options = [
    ...
    'trade_type'       => 'JSAPI',
    'notify_url'       => 'http://www.wdja.net/user/wxpay_notify.php',
...
function wxpayjs($out_trade_no,$subject,$total_amount,$body,$notify_url='') {
...
function alipayjs($out_trade_no,$subject,$total_amount,$body,$notify_url='') {
    global $alipayjs_id,$alipayjs_key,$alipayjs_notify;
    $mchid = $alipayjs_id;
    $key = $alipayjs_key;
    if($notify_url == '') $notify_url = $alipayjs_notify;
    $total_amounts = $total_amount*100;
    $data = [
        "mchid" => $mchid,
        "type" => 'alipay',//支付宝交易传值：alipay ，微信支付无需此字段
        "total_fee" => $total_amounts,//金额。单位：分
        "out_trade_no" => $out_trade_no,//用户端自主生成的订单号
        "body" => $subject,//订单标题
        "notify_url" => $notify_url,
