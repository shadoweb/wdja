...
function pp_get_topics($ffsid,$lng,$fsid)
{
  ...
  $tsqlstr = "select * from $ndatabase where " . ii_cfnames($nfpre, 'fsid') . "=$tffsid and " . ii_cfnames($nfpre, 'type') . "=0 and " . ii_cfnames($nfpre, 'lng') . "='$tlng' and " . ii_cfnames($nfpre, 'hidden') . "=0 order by " . ii_cfnames($nfpre, 'time') . " asc";
...
function wdja_cms_admin_manage_edit()
{
  ...
  if ($trs)
  {
    ...
    $tmpstr = str_replace('{$type}', $trs[ii_cfname('type')], $tmpstr);
    $tmpstr = str_replace('{$upsimg}', $nupsimg, $tmpstr);
    $tmpstr = str_replace('{$upsimgs}', $nupsimgs, $tmpstr);
    $tmpstr = ii_creplace($tmpstr);
    return $tmpstr;
...
function wdja_cms_admin_manage_list()
{
  ...
  if (is_array($trsary))
  {
    foreach($trsary as $trs)
    {
      ...
      if($ttype == 0) $url = '/'.$ngenre.'/'.ii_iurl('list',$tid, $nurltype, 'folder=' . $ncreatefolder . ';filetype=' . $ncreatefiletype);
      else $url = '/'.$ngenre.'/'.ii_iurl('detail',$tid, $nurltype, 'folder=' . $ncreatefolder . ';filetype=' . $ncreatefiletype);
