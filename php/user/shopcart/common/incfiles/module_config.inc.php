...
function wdja_cms_module_buydisp()
{
  ...
  $tprices = 0;
    foreach ($tidsAry as $val)
    {
      ...
      $tprice = api_get_userGroupPrice($tid,$tugid);//最新价格
      //$tprice = ii_get_lrstr($val, ':', 'right');
      $tprices = $tprices + $tnum*$tprice;
      if ($tid != 0 && $tnum != 0)
      {
        ...
        foreach ($tary as $key => $val)
        {
          ...
          if ($tid != 0)
          {
            ...
            if ($trs2)
            {
              ...
              $tmptstr = $tmpastr;
              global $variable;
              $nurltype = $variable[ii_cvgenre($nshop) . '.nurltype'];
              $ncreatefolder = $variable[ii_cvgenre($nshop) . '.ncreatefolder'];
              $ncreatefiletype = $variable[ii_cvgenre($nshop) . '.ncreatefiletype'];
              $turl = '/'.$nshop.'/'.ii_iurl('detail',$trs2[$tidfield], $nurltype, 'folder=' . $ncreatefolder . ';filetype=' . $ncreatefiletype);
    ...
    foreach ($tcookiesAry as $key => $val)
    {
      ...
      if ($tid != 0)
      {
        ...
        if ($trs)
        {
          ...
          $ncreatefolder = $variable[ii_cvgenre($nshop) . '.ncreatefolder'];
          $ncreatefiletype = $variable[ii_cvgenre($nshop) . '.ncreatefiletype'];
          $turl = '/'.$nshop.'/'.ii_iurl('detail',$trs[$nidfield], $nurltype, 'folder=' . $ncreatefolder . ';filetype=' . $ncreatefiletype);
